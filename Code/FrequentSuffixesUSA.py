# -*- coding: utf-8 -*-
"""
Get most frequent suffixes for the USA.
"""
...
df_gazetteer = pd.read_csv('/BhugolCode/Data/Gazetteers/USA/GazetteerUSAPopulatedPlace.csv', encoding='latin-1')
placename = df_gazetteer['FEATURE_NAME'].str.lower().tolist() + list(set(df_gazetteer['COUNTY_NAME'].str.lower().tolist()))
placenames = [str(place).strip() for place in placename]
area_names = [re.sub('\s|\([^()]*\)', '', place) for place in placenames]
