...
def get_NER_5WNER(doc, NER,placeNames):
    """
    Use SANS (NER for India) to identify location names in text.
    """
    input_ner = []
    tags = []
    for sent in doc:
...
def main():
    ...
    for suff in suffixes:
        ...
        for place in groundtruth:
            ...
            if len(pts) == 1:#if only one co-occurring place name exist, the algorithm uses the place name to geocode.
