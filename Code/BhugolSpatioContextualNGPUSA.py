...
def main():
    ...
    groundtruth = pd.read_csv('/BhugolCode/Data/GroundTruth/USA/NPlatlongUSA.csv')['Placename'].tolist()
    groundtruth_lats = pd.read_csv('/BhugolCode/Data/GroundTruth/USA/NPlatlongUSA.csv')['Lat'].tolist()
    groundtruth_longs = pd.read_csv('/BhugolCode/Data/GroundTruth/USA/NPlatlongUSA.csv')['Long'].tolist()
    ...
    for place in groundtruth:
        ...
        if len(pts) == 0: #if no co-occurring place name exist, the algorithm cannot geocode the non-gazetteer place names. So, the geographic centroid of USA is used to geocode.
